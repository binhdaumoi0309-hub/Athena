...
export function StructuredMessage({ message }: { message: AssistantMessage }) {
  return <div className={styles.content}>
    ...
    {message.actions.length>0&&<div className={styles.actions}>{message.actions.map((action)=>action.href?.startsWith('/')?<Link key={action.id} to={action.href}>{action.label}</Link>:<a key={action.id} href={action.href} target="_blank" rel="noopener noreferrer">{action.label}</a>)}</div>}
    {message.citations.length>0&&<div className={styles.citations}>{message.citations.map((citation)=><a key={citation.url} href={citation.url} target="_blank" rel="noopener noreferrer"><ExternalLink size={12}/>{citation.title}</a>)}</div>}
  </div>;
}
